...
def create_language_classifier(
    languages: list[str], select_k_features_per_language: int
) -> None:
    """
    ...
    dataset = create_dataset(only_title=False, languages=languages)
    ...
    cloudpickle.dump(pipeline, open(os.path.dirname(os.path.realpath(__file__)) + "/language_classifier.pickle", "wb"))
