...
class ContentTag(str, Enum):
    REPLACE = 'r',  # Element should be replaced according to tag. If it has children, the option applies to them
    DELETE = 'd',  # Element should be deleted
    DELETE_LEAVE_ONE = 'o',  # Element should be deleted, but at least one must remain (for lists)
